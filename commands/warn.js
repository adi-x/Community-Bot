...
exports.handle = function(data, message, local) {
  ...
  var authorId = new String(message.author.id);
  var Now = 'Warn', i, logId, Selected = false;
  var Messages = {
    [1] : 'Introduce the reason of the warning',
    [2] : 'Introduce the expiration date of the warning',
  };
  Messages.length = 2;
...
function GenerateMessage(data, authorId, CA) {
  ...
  var info = CA[authorId].Warn;
  embed.setTitle("Warning notice for " + F.NicknameFromTag(data, info.Target.tag));
  embed.setColor("#FF7F50");
  embed.setDescription(info[1]);
  embed.addField("Warning Reason",info[2]);
