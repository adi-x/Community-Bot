...
exports.handle = function(data, message, local) {
  ...
  var authorId = new String(message.author.id);
  var Now = 'Fire', i, logId, Selected = false;
  var Messages = {
    [1] : 'Introduce the reason of the firing',
  };
  Messages.length = 1;
...
function GenerateMessage(data, authorId, CA) {
  ...
  var info = CA[authorId].Fire;
  embed.setTitle("Employment dismissal notice for " + F.NicknameFromTag(data, info.Target.tag));
  embed.setColor("#DC143C");
  embed.setDescription(info[1]);
  embed.addField("Firing Reason",info[2]);
  embed.setFooter("\nFOOTER");
  return embed;
}
