...
exports.handle = function(data, message, local) {
  ...
  var authorId = new String(message.author.id);
  var Now = 'Demote', i, logId, Selected = false;
  var Messages = {
    [1] : 'Introduce the reason of the demotion',
    [2] : 'Introduce the OLD RANK of the demoted person',
    [3] : 'Introduce the NEW RANK of the demoted person',
  };
  Messages.length = 3;
...
function GenerateMessage(data, authorId, CA) {
  ...
  var info = CA[authorId].Demote;
  embed.setTitle("Demotion notice for " + F.NicknameFromTag(data, info.Target.tag));
  embed.setColor("#4169E1");
  embed.setDescription(info[1]);
  embed.addField("Demotion Reason",info[2]);
  embed.addField("Old rank",info[3]);
  embed.addField("New rank",info[4]);
  embed.setFooter("\nFOOTER");
  return embed;
}
